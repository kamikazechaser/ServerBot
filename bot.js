...
jsonWatcher.watch('netstat.json');

var results = jsonWatcher.get() 

bot.init();

bot.command('ping', 'Execute Speed-Test', false, (args, message) => {
message.from.sendMessage(`<code>Speed Test Started!</code>`, {parse_mode: 'HTML'})
...
bot.command('disk', 'Disk Statistics', false, (args, message) => {
    message.reply(`<b>Free Space:</b> <code>${Math.floor((info.free) / 1048576)} MB  ${Math.floor((info.free / info.total) * 100)}% free</code>\n<b>Used Space:</b> <code>${Math.floor((info.total - info.free ) / 1048576)} MB ${Math.floor(((info.total - info.free) / info.total) * 100)}% used</code>`, {parse_mode: 'HTML'}); 
});
});
